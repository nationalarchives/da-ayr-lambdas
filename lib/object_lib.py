...
def raise_error_if_object_exists(bucket, object_name):
    """
    Raise a ValueError if `object` exists in `bucket`.
    """
    logger.info(
        f'raise_error_if_object_exists start: checking "{object_name}" does '
        f'not already exist in "{bucket}"')

    if s3_object_exists(bucket, object_name):
        raise ValueError(
            f'Copy not allowed; "{object_name}" already exists in bucket '
            f'"{bucket}"')
...
def get_s3_object_presigned_url(bucket, key, expiry):
    """
    Return a presigned URL for `key` in `bucket` with the specified
    `expiration` seconds.
    """
    logger.info(
